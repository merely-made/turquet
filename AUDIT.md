...
Turquet 0.15.0. It inventories every exported calculation in the crate as of
...
`turquet::compat`. Their source modules are private in 0.15.0. The Turquet-era
...
| `events::{LocalSolarEclipseSearch, LocalSolarEclipseKind, LocalSolarEclipseContactKind, LocalSolarEclipseContact, LocalSolarEclipseGeometry, LocalSolarEclipseVisibility, LocalSolarEclipseCircumstances, local_solar_eclipse_circumstances}` | Around each new-moon phase, composes one provider-neutral WGS84 airless observer transform for the Sun and Moon, minimizes fixed-spherical-limb outer clearance, then returns bounded greatest and C1--C4 local contact intervals. It classifies strict local disk overlap as partial, annular, or total and retains observer, search, geometry/provider/transform models, and EOP authority/snapshot. Visibility means only whether the physical solar upper limb is above the airless horizon at greatest eclipse. Lunar limb relief, refraction, terrain, obstruction, weather, eye safety, and visibility windows are outside the contract. | Analytical Boston partial, Dallas total, and Albuquerque annular maxima are within 11.630 seconds of published local circumstances. A 2,950-row five-minute DE441/Horizons fixture covers those classes, Galway's below-horizon geometry, and Cape Town's no-overlap control. All fixture events agree in class, expected ordered contact count, and horizon state; direct Horizons Sun altitude differs by at most 0.00648 degrees. Validation, short-span, position/EOP/identity/transform failures, and one-second result intervals are tested. | measured |
| `events::{LunarIllumination, LunarIlluminationError, GEOCENTRIC_LUNAR_ILLUMINATION, lunar_illumination_at}` | One TT epoch plus provider-supplied same-frame Sun and Moon apparent geocentric states to fraction, Sun-Moon elongation, phase angle at the Moon, and all three triangle legs. The result retains revisioned triangle and provider model/snapshot; a returned state with another TT epoch, a zero range, coincident bodies, a non-finite triangle, or a provider error is typed. It deliberately does not make a topocentric, limb-profile, atmospheric, or visibility claim. | Thirty paired DE441/Horizons rows cover 12 hours before through 12 hours after April 2024 new, first-quarter, and full Moon. The raw Horizons fixture triangle stays within 0.000010 fraction, 0.001 degrees phase/elongation of its independently reported values; the analytical fraction stays within 0.000015. Unit tests cover new/quarter/full ordering, provenance, both position errors, returned-state epoch mismatch, zero distance, coincidence, and overflow. | measured |
...
at `turnstone/design_docs/2026-08-26_sky_home_timeline_plan.md`. T4j now adds
observer solar contacts and the deliberately narrow airless-horizon fact at
greatest eclipse. General visibility, illumination/distance extrema, and
twilight remain explicit in `ROADMAP.md`.
